...
def create_dataloader():
    with open('./train_data_explicit_h.pkl', 'rb') as f:
    ...
    with open('./validation_data_explicit_h.pkl', 'rb') as f:
        val_data = pk.load(f)
    ...
    train_dataset = Data.TensorDataset(train_element, train_charge, train_aromatic, train_adj, train_label, train_mask)
    val_dataset = Data.TensorDataset(val_element, val_charge, val_aromatic, val_adj, val_label, val_mask)
...
def train(m ,criterion, optimizer, train_loader):
    ...
    for element, charge, aromatic, adj, label, mask in train_loader:
...
def evaluate(m, val_loader):
    m.eval()
    for element, charge, aromatic, adj, label, mask in val_loader:
        element = element.to(torch.device('cuda'))
        charge = charge.to(torch.device('cuda'))
        aromatic = aromatic.to(torch.device('cuda'))
        adj = adj.float().to(torch.device('cuda'))
